...
const isCustomerActive = (item) => {
  const status = getGlobalStatusStr(item);
  if (status.includes('AKTIF') || status === 'DONE') return true;
  const akt = String(item.aktivasi || item.statusAktivasi || '').toLowerCase();
  return akt.includes('sudah') || akt === 'aktif';
};
...
export default function ActivationCalendarTable({ pelangganData = [], onGoToDatabase }) {
  const now = new Date();
  ...
  // Kalkulasi matriks aktivasi per stasiun per tanggal
  const { matrixData, stationTotals, dailyTotals, grandTotal, maxCellCount } = useMemo(() => {
    const matrix = {};
    const stTotals = {};
    ...
    (pelangganData || []).forEach(item => {
      if (!isCustomerActive(item)) return;
      ...
      if (matrix[st] && matrix[st][tAkt]) {
        matrix[st][tAkt].push(item);
        stTotals[st] = (stTotals[st] || 0) + 1;
        dayTotals[tAkt] = (dayTotals[tAkt] || 0) + 1;
        totalAll++;

        if (matrix[st][tAkt].length > maxCell) {
          maxCell = matrix[st][tAkt].length;
        }
      }
    });
  ...
  // Helper pewarnaan cell badge aktivasi
  ...
  // Kalkulasi total kabel precon untuk modal
  const totalCableMeters = useMemo(() => {
    if (!selectedCell || !selectedCell.items) return 0;
    let total = 0;
    selectedCell.items.forEach(item => {
      let str = String(item.kabelPrecon || '0').toLowerCase();
  ...
  const filteredModalItems = useMemo(() => {
    ...
    return selectedCell.items.filter(cust => {
      return (
        String(cust.idPelanggan || '').toLowerCase().includes(q) ||
        String(cust.namaPelanggan || '').toLowerCase().includes(q) ||
        String(cust.petugasAktivasi || '').toLowerCase().includes(q) ||
        String(cust.petugasIkr || '').toLowerCase().includes(q) ||
        String(cust.odpAktual || '').toLowerCase().includes(q) ||
        String(cust.alamat || '').toLowerCase().includes(q) ||
        String(cust.kabelPrecon || '').toLowerCase().includes(q)
      );
    });
  }, [selectedCell, searchQuery]);

  // Ekspor rincian aktivasi tanggal tersebut ke Excel
  const handleExportModalExcel = () => {
    if (!selectedCell || !selectedCell.items || !selectedCell.items.length) return;
    try {
      const headers = ['NO', 'ID PELANGGAN', 'NAMA PELANGGAN', 'STASIUN', 'TANGGAL AKTIVASI', 'ODP', 'KABEL PRECON', 'ALAMAT', 'PETUGAS AKTIVASI', 'STATUS'];
      const rows = selectedCell.items.map((cust, idx) => [
        idx + 1,
        cust.idPelanggan || '-',
        cust.namaPelanggan || 'Tanpa Nama',
        selectedCell.station,
        selectedCell.date,
        cust.odpAktual || '-',
        cust.kabelPrecon || '-',
        cust.alamat || '-',
        cust.petugasAktivasi ? `@${String(cust.petugasAktivasi).replace(/^@/, '')}` : '-',
        'AKTIF'
      ]);
      const ws = XLSX.utils.aoa_to_sheet([headers, ...rows]);
      ws['!cols'] = [{ wch: 6 }, { wch: 15 }, { wch: 25 }, { wch: 18 }, { wch: 16 }, { wch: 22 }, { wch: 14 }, { wch: 35 }, { wch: 20 }, { wch: 12 }];
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Rincian Aktivasi');
      const cleanStation = String(selectedCell.station).replace(/[^a-zA-Z0-9]/g, '_');
      XLSX.writeFile(wb, `Rincian_Aktivasi_${cleanStation}_${selectedCell.date}.xlsx`);
    } catch (err) {
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden flex flex-col mb-6">
      ...
      <div className="p-4 sm:p-5 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-start sm:items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] sm:text-xs text-slate-500 mt-0.5">
              Matriks sebaran aktivasi harian untuk seluruh stasiun di sepanjang jalur operasional
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center flex-wrap gap-2 self-start md:self-auto">
          ...
          <div className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-50 border border-slate-200 text-xs font-semibold text-slate-700">
            ...
            <span className="text-[10px] text-slate-400 uppercase font-medium">Aktivasi</span>
          </div>
            ...
            <tr>
              ...
              {daysList.map(d => {
                ...
                return (
                  <th
                    key={d.day}
                    className={`sticky top-0 z-20 px-1 py-2 min-w-[40px] max-w-[44px] text-center border-b border-r border-slate-200 transition-colors select-none ${d.isToday
            ...
            {STATIONS_ORDER.map((st, sIdx) => {
              ...
              return (
                <tr key={st} className="group hover:bg-slate-50/80 transition-colors">
                  ...
                  {/* KOLOM TANGGAL 1 s/d Akhir Bulan */}
                  {daysList.map(d => {
                    const customers = matrixData[st]?.[d.dateStr] || [];
                    const count = customers.length;
                    return (
                      <td
                        key={d.day}
                        className={`text-center p-0.5 border-b border-r border-slate-100/90 transition-colors ${d.isToday ? 'bg-blue-50/20' : d.isWeekend ? 'bg-slate-50/40' : ''
                          }`}
                      >
                        {count > 0 ? (
                          <button
                            ...
                            title={`${st} - ${d.day} ${MONTH_NAMES[currentMonth]}: ${count} aktivasi (Klik untuk rincian)`}
                          >
                            {count}
                          </button>
                        ) : (
                  ...
                  {/* TOTAL BARIS STASIUN (STICKY RIGHT - SOLID OPAQUE DENGAN GARIS PERBARIS) */}
                  <td
            ...
            <tr className="sticky bottom-0 z-20 bg-slate-100 border-t-2 border-slate-300 font-bold">
              ...
              {daysList.map(d => {
                ...
                return (
                  <td
                    key={d.day}
                    className={`text-center py-2 px-0.5 border-r border-slate-200 text-[11px] font-black ${d.isToday ? 'bg-blue-100/90 text-blue-900' : 'bg-slate-100 text-slate-700'
                      }`}
      ...
      <div className="p-3 sm:px-5 bg-white border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-slate-500">
        <div className="flex items-center gap-4 flex-wrap">
          <span>
            Total Bulan Ini: <strong className="text-slate-800">{grandTotal} Pelanggan</strong>
          </span>
        ...
        <div className="text-[11px] text-slate-400">
          *Data bersumber dari pelanggan berstatus AKTIF pada tanggal aktivasi terkait.
        </div>
      </div>

      {/* MODAL POPUP RINCIAN AKTIVASI HARIAN PER STASIUN (PORTAL KE BODY DENGAN BACKDROP BLUR PENUH & TERKUNCI) */}
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-2.5 sm:p-6">
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-4xl lg:max-w-5xl overflow-hidden flex flex-col max-h-[85vh] sm:max-h-[90vh] relative z-10 animate-modal">
            ...
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/90">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-emerald-100/90 border border-emerald-200 text-emerald-700 flex items-center justify-center font-bold shadow-xs shrink-0">
                  <Icon name="check-circle" size={20} />
                </div>
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="font-bold text-base text-slate-800">
                      Rincian Aktivasi: {selectedCell.station}
                    </h3>
                  ...
                  <div className="flex items-center gap-3 mt-1 text-xs text-slate-500 flex-wrap">
                    <span>
                      Total Selesai: <strong className="text-emerald-700 font-bold">{selectedCell.items.length} Pelanggan</strong>
                    </span>
                    {totalCableMeters > 0 && (
                      <>
                        <span className="text-slate-300">•</span>
                        <span>
                          Total Precon: <strong className="text-blue-700 font-bold">{totalCableMeters} meter</strong>
                        </span>
                      </>
                    )}
                  </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  title="Unduh rincian hari ini ke format Excel"
            ...
            <div className="px-5 py-3 border-b border-slate-100 bg-white flex flex-col sm:flex-row items-center justify-between gap-3">
              <div className="relative w-full sm:w-80">
                <input
                  ...
                  placeholder="Cari ID, nama, ODP, alamat, atau petugas..."
              ...
              <div className="text-xs text-slate-500 self-end sm:self-auto font-medium">
                Menampilkan <strong className="text-slate-800 font-bold">{filteredModalItems.length}</strong> dari {selectedCell.items.length} pelanggan
              </div>
            </div>

            {/* Modal Body: Tabel Data Pelanggan Resmi */}
            <div className="overflow-x-auto overflow-y-auto max-h-[54vh] flex-1">
              <table className="w-full text-left border-collapse text-xs whitespace-nowrap min-w-[720px]">
                <thead className="bg-slate-50 text-slate-500 font-bold border-b border-slate-200 sticky top-0 z-10 text-[11px]">
                  <tr>
                    ...
                    <th className="px-4 py-3">ALAMAT / ODP</th>
                    <th className="px-4 py-3 text-center">KABEL</th>
                    <th className="px-4 py-3">PETUGAS AKTIVASI</th>
                  ...
                  {filteredModalItems.length === 0 ? (
                    <tr>
                      <td colSpan="7" className="px-6 py-12 text-center text-slate-400">
                        <p className="font-medium">Tidak ada data pelanggan yang cocok dengan kata kunci pencarian.</p>
                      </td>
                    </tr>
                  ) : (
                    filteredModalItems.map((cust, idx) => (
                      <tr key={cust.idPelanggan || idx} className="hover:bg-blue-50/40 transition-colors">
                        ...
                        <td className="px-4 py-3 text-slate-600">
                          ...
                          {cust.alamat ? (
                            <span className="text-[11px] text-slate-400 block truncate max-w-[280px]" title={cust.alamat}>
                              {cust.alamat}
                            </span>
                          ) : (
                            <span className="text-slate-300">-</span>
                        ...
                        <td className="px-4 py-3">
                          {cust.petugasAktivasi ? (
                            <span className="px-2.5 py-1 rounded-lg text-xs font-bold bg-blue-50 text-blue-700 border border-blue-100 inline-flex items-center gap-1">
                              @{String(cust.petugasAktivasi).replace(/^@/, '')}
                            </span>
                          ) : (
                            <span className="text-slate-400 italic">Petugas -</span>
                          )}
                        </td>
                        <td className="px-4 py-3 text-center">
            ...
            {/* Modal Footer */}
            <div className="px-5 py-3.5 border-t border-slate-100 bg-slate-50/70 flex items-center justify-between text-xs">
              <div className="flex items-center gap-2 text-slate-500 text-xs">
                <span>Stasiun: <strong className="text-slate-700 font-bold">{selectedCell.station}</strong></span>
                <span className="text-slate-300">•</span>
                <span>Total: <strong className="text-blue-600 font-bold">{selectedCell.items.length} Pelanggan</strong></span>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
