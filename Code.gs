...
const STATION_DB_MAP = {
  ...
  "Tawang": { id: "1HmG71UGj2r7lOB5XzDzWTETMjYKoij-X8ofCw_XrEdk", sheetName: "Tawang" },
...
function getDashboardData() {
  try {
    ...
    validRows.forEach(row => {
      if (row.tanggalRegistrasi) {
        let dateStr = String(row.tanggalRegistrasi).trim();
        if (dateStr.includes('T')) {
          try {
            let d = new Date(dateStr);
            dateStr = Utilities.formatDate(d, "GMT+7", "dd/MM/yyyy HH:mm:ss");
          } catch (err) { }
        }
        listRegistrasi.push({
          idPelanggan: row.idPelanggan,
          namaPelanggan: row.namaPelanggan || 'Tanpa Nama',
          stasiun: row.stasiun || 'Tanpa Stasiun',
          tanggal: dateStr
        });
      }
    });
    ...
    if (dashboardSheet) {
      ...
      try {
        ...
        poValues.forEach(r => {
          ...
          // F = r[5] (Reguler), G = r[6] (Terbangun sum), H = r[7] (Percepatan)
          const hpReg = parseIndoNum(r[5]);
          const hpTerbangunCol = parseIndoNum(r[6]);
          ...
          listDetailPo.push({
            ...
            hpReguler: hpReg,
            hpPercepatan: hpPerc,
            hpTerbangun: hpTerbangunCol > 0 ? hpTerbangunCol : (hpReg + hpPerc),
...
function updatePelangganData(payload) {
  try {
    ...
    const getColIdx = (keywords, exclude = []) => headers.findIndex(h => {
      const s = String(h).toLowerCase();
      if (exclude.some(ex => s.includes(ex))) return false;
    ...
    const idxKendala = getColIdx(['keterangan kendala', 'issue kendala', 'kendala', 'issue'], ['pelapor', 'reporter']);
    const idxReporterKendala = getColIdx(['pelapor kendala', 'reporter kendala', 'pelapor', 'reporter']);
    ...
    if (idxKendala !== -1 && payload.issueKendala !== undefined) dbSheet.getRange(targetRow, idxKendala + 1).setValue(payload.issueKendala);
    if (idxReporterKendala !== -1 && payload.reporterKendala !== undefined) dbSheet.getRange(targetRow, idxReporterKendala + 1).setValue(payload.reporterKendala);
  ...
  try {
    ...
    if (dashboardSheet) {
      try {
        ...
        var headerN = String(headersRow5[0] || '').trim().toLowerCase(); // Kolom N

        if (headerN.includes("id")) {
          isNewDashboardSystemActive = true;
        } else {
          // Cek kolom O (15) atau P (16)
          var headerO = String(dashboardSheet.getRange(5, 15, 1, 1).getValue()).trim().toLowerCase();
          var headerP = String(dashboardSheet.getRange(5, 16, 1, 1).getValue()).trim().toLowerCase();

          if (headerO.includes("id") || headerP.includes("id")) {
            isNewDashboardSystemActive = true;
          }
        }
      } catch (e) {
        Logger.log("[Fast Dashboard] Error mendeteksi formula kolom: " + e.message);
      }
    }
    ...
    if (isNewDashboardSystemActive) {
      ...
      // Deteksi offset kolom (apakah di N, O, atau P)
      var colOffset = 15; // Default ke O (sesuai posisi sekarang)
      try {
        var headerN = String(dashboardSheet.getRange(5, 14, 1, 1).getValue()).toLowerCase();
        var headerO = String(dashboardSheet.getRange(5, 15, 1, 1).getValue()).toLowerCase();
        var headerP = String(dashboardSheet.getRange(5, 16, 1, 1).getValue()).toLowerCase();

        if (headerO.includes("id")) {
          colOffset = 15; // O
        } else if (headerP.includes("id")) {
          colOffset = 16; // P
        } else if (headerN.includes("id")) {
          colOffset = 14; // N
        }
      } catch (e) { }

      // 1. Baca Registrasi Hari Ini
      try {
        var regData = dashboardSheet.getRange(6, colOffset, 95, 4).getValues();
        regData.forEach(r => {
      ...
      // 2. Baca Aktivasi Hari Ini
      try {
        var aktivasiData = dashboardSheet.getRange("T6:AD").getValues();
      ...
      // 3. Baca IKR Hari Ini
      try {
        var ikrData = dashboardSheet.getRange("AF6:AO").getValues();
        ikrData.forEach(r => {
      ...
      // 4. Baca Kendala Hari Ini
      try {
        var kendalaData = dashboardSheet.getRange(6, 62, 95, 6).getValues(); // BJ6:BO
        kendalaData.forEach(r => {
          ...
          // Jika kolom ke-5 ternyata Date atau berformat GMT+ (tanggal), tukar dengan tglVal
          if (issueVal instanceof Date || (typeof issueVal === 'string' && (issueVal.includes('GMT+') || issueVal.includes('Waktu Indonesia')))) {
            var temp = issueVal;
            issueVal = tglVal;
            tglVal = temp;
          }

          var tglKendalaVal = tryParseDate(tglVal);
          var tglKendalaStr = tglKendalaVal ? Utilities.formatDate(tglKendalaVal, "GMT+7", "yyyy-MM-dd'T'HH:mm:ss") : String(tglVal || '').trim();
          var issueStr = String(issueVal || '').trim();
      ...
      // 5. Baca Visit Log mentah langsung dari sheet Dashboard (untuk Tab Visit / Gangguan)
      try {
        var visitRange = dashboardSheet.getRange("AQ6:BI");
        var vData = visitRange.getValues();
        var vRichText = visitRange.getRichTextValues();
        var vFormulas = visitRange.getFormulas();

        for (var v = 0; v < vData.length; v++) {
          var vIdRaw = String(vData[v][1] || '').trim();
          if (!vIdRaw || vIdRaw.toLowerCase().includes("id pelanggan") || vIdRaw.toLowerCase().includes("belum") || vIdRaw.toLowerCase().includes("tidak")) continue;
      ...
      if (sheet) {
        ...
        const idxReporterKendala = getColIdx(['pelapor kendala', 'reporter kendala', 'pelapor', 'reporter']);
        const idxIssueKendala = getColIdx(['keterangan kendala', 'issue kendala', 'kendala', 'issue'], ['pelapor', 'reporter']);
        const idxAlamat = getColIdx(['alamat']);
    ...
    if (dashboardSheet) {
      try {
        var numCols = 14;
        try {
          var checkBounds = dashboardSheet.getRange(21, 1, 55, 14).getValues();
        } catch (e) {
          numCols = 13; // Fallback jika sheet hanya punya 13 kolom
        }
        var poRange = dashboardSheet.getRange(21, 1, 55, numCols);
        var poValues = poRange.getValues();
        poValues.forEach(r => {
          var stasiun = String(r[1] || r[0] || '').trim();
          if (!stasiun || stasiun.toLowerCase() === 'stasiun' || stasiun.toLowerCase() === 'total' || stasiun.toLowerCase().includes('no')) return;

          var perfRaw = numCols === 14 ? (r[13] || 0) : (r[12] || 0);
          var perf = 0;
          if (typeof perfRaw === 'number') {
            perf = perfRaw <= 1 ? parseFloat((perfRaw * 100).toFixed(2)) : parseFloat(perfRaw.toFixed(2));
          } else {
          ...
          const hpReg = parseInt(String(r[5] || '').replace(/[^0-9]/g, '')) || 0;
          const hpTerbangunCol = parseInt(String(r[6] || '').replace(/[^0-9]/g, '')) || 0;
          const hpPerc = parseInt(String(r[7] || '').replace(/[^0-9]/g, '')) || 0;
          ...
          listDetailPo.push({
            ...
            hpTerbangun: hpTerbangunCol > 0 ? hpTerbangunCol : (hpReg + hpPerc),
            hpPercepatan: hpPerc,
            totalAktivasiHc: parseInt(String(r[8] || '').replace(/[^0-9]/g, '')) || 0,
            hcAktif: parseInt(String(r[9] || '').replace(/[^0-9]/g, '')) || 0,
            suspend: parseInt(String(r[10] || '').replace(/[^0-9]/g, '')) || 0,
            readyToDismantle: parseInt(String(r[11] || '').replace(/[^0-9]/g, '')) || 0,
            dismantled: parseInt(String(r[12] || '').replace(/[^0-9]/g, '')) || 0,
            performaHc: perf
          });
        });
      } catch (e) {
...
// ==========================================
// HELPER: HITUNG JARAK & ANALISA COVERAGE ODP
// ==========================================
...
function ekstrakKoordinatPelanggan(lat, lng, rawKoordinat) {
  if (lat && lng && !isNaN(parseFloat(lat)) && !isNaN(parseFloat(lng))) {
    return { lat: parseFloat(lat), lng: parseFloat(lng) };
  }
  if (!rawKoordinat) return null;
  const str = String(rawKoordinat).trim();
...
function normalisasiNamaStasiun(nama) {
  if (!nama) return "";
  let s = String(nama).trim();
  const daftarStasiun = [
    "Wadu", "Randublatung", "Sulur", "Kradenan", "Brumbung",
    "Alastua", "Tawang", "Kaliwungu", "Kalibodri", "Weleri",
    "Krengseng", "Doplang", "Panunggalan", "Jambon", "Gambringan",
    "Sedadi", "Karangsono", "Gubug"
  ];
  for (let st of daftarStasiun) {
    if (s.toLowerCase().includes(st.toLowerCase())) {
      return st;
    }
  }
  return s.replace(/^stasiun\s+/i, '').replace(/^semarang\s+/i, '').trim();
}

function getOdpDataByStation(stasiun, ssMaster, odpCacheMap) {
  const cleanStation = normalisasiNamaStasiun(stasiun);
  const stKey = cleanStation.toLowerCase().trim();
  if (odpCacheMap && odpCacheMap[stKey]) return odpCacheMap[stKey];

  let odps = [];

  // 1. Coba ambil langsung dari Supabase (Cepat & Akurat)
  try {
    if (typeof SUPABASE_URL !== 'undefined' && typeof SUPABASE_KEY !== 'undefined') {
      let queryUrl = SUPABASE_URL + "/rest/v1/odp?select=kode_odp,label,latitude,longitude,kapasitas,port_terpakai,stasiun";
      if (cleanStation && cleanStation !== "-") {
        queryUrl += "&stasiun=ilike.*" + encodeURIComponent(cleanStation) + "*";
      }
      let res = UrlFetchApp.fetch(queryUrl, {
    ...
    Logger.log("[Supabase ODP Fetch Error]: " + e.message);
  }

  // 2. Fallback: Ambil dari Sheet List ODP
  if (odps.length === 0 && ssMaster) {
    try {
      const odpSheet = ssMaster.getSheetByName("List ODP") || ssMaster.getSheetByName("list_odp") || ssMaster.getSheetByName("Cache_ODP");
      if (odpSheet) {
        const vals = odpSheet.getDataRange().getValues();
        if (vals.length > 1) {
          const headers = vals[0].map(h => String(h).trim().toLowerCase());
          const idxLabel = headers.findIndex(h => h.includes('label') || h.includes('nama'));
          const idxKode = headers.findIndex(h => h.includes('kode') || h.includes('odp'));
          const idxLat = headers.findIndex(h => h.includes('lat'));
          const idxLng = headers.findIndex(h => h.includes('long') || h.includes('lng') || h.includes('lon'));
          const idxStasiun = headers.findIndex(h => h.includes('stasiun'));
          const idxKap = headers.findIndex(h => h.includes('kapasitas') || h.includes('port'));

          for (let i = 1; i < vals.length; i++) {
            let rowStasiun = idxStasiun !== -1 ? String(vals[i][idxStasiun] || "") : "";
            let rowClean = normalisasiNamaStasiun(rowStasiun).toLowerCase();
            if (stKey && rowClean !== stKey) {
              continue;
            }
            odps.push({
              label: idxLabel !== -1 ? vals[i][idxLabel] : "",
              kode_odp: idxKode !== -1 ? vals[i][idxKode] : "",
              latitude: idxLat !== -1 ? vals[i][idxLat] : "",
              longitude: idxLng !== -1 ? vals[i][idxLng] : "",
              kapasitas: idxKap !== -1 ? vals[i][idxKap] : 8,
              port_terpakai: 0,
              stasiun: rowStasiun
            });
          }
      ...
      Logger.log("[Sheet ODP Fetch Error]: " + errSheet.message);
    }
  }

  if (odpCacheMap) odpCacheMap[stKey] = odps;
  return odps;
}

function analisaCoverageOdp(stasiun, lat, lng, rawKoordinat, ssMaster, odpCacheMap) {
  const coords = ekstrakKoordinatPelanggan(lat, lng, rawKoordinat);
  if (!coords) {
    return {
      isCovered: false,
      formattedText: `📡 <b>COVERAGE ALPRO:</b>\n⚪ <b>Status :</b> Belum dapat dicek (Koordinat kosong)`
    };
  }

  const odps = getOdpDataByStation(stasiun, ssMaster, odpCacheMap);
  if (!odps || odps.length === 0) {
    return {
      isCovered: false,
      formattedText: `📡 <b>COVERAGE ALPRO:</b>\n⚪ <b>Status :</b> Data ODP stasiun ${stasiun} tidak ditemukan`
    };
  }

  // Hitung jarak garis lurus ke seluruh ODP
  let candidates = [];
  for (let i = 0; i < odps.length; i++) {
    let o = odps[i];
    let oLat = parseFloat(o.latitude);
    let oLng = parseFloat(o.longitude);
    if (isNaN(oLat) || isNaN(oLng) || (oLat === 0 && oLng === 0)) continue;

    let straightDist = haversineMeters(coords.lat, coords.lng, oLat, oLng);
    candidates.push({
      ...o,
      oLat: oLat,
      oLng: oLng,
  ...
  if (candidates.length === 0) {
    return {
      isCovered: false,
      formattedText: `📡 <b>COVERAGE ALPRO:</b>\n🔴 <b>Status :</b> Tidak ada koordinat ODP valid di stasiun ini`
    };
  }

  // Urutkan dari yang terdekat garis lurus
  candidates.sort((a, b) => a.straightDist - b.straightDist);

  // Ambil top 3 ODP terdekat untuk diuji jarak jalannya via OSRM
  let topCandidates = candidates.slice(0, 3);
  let bestOdp = topCandidates[0];
  let finalRoadDist = null;

  try {
    for (let c of topCandidates) {
      let osrmUrl = `https://router.project-osrm.org/route/v1/driving/${coords.lng},${coords.lat};${c.oLng},${c.oLat}?overview=false`;
      let res = UrlFetchApp.fetch(osrmUrl, { muteHttpExceptions: true });
      if (res.getResponseCode() === 200) {
        let json = JSON.parse(res.getContentText());
        if (json.routes && json.routes.length > 0) {
          c.roadDist = Math.round(json.routes[0].distance);
        }
      }
    }

    let withRoads = topCandidates.filter(c => typeof c.roadDist === 'number');
    if (withRoads.length > 0) {
      withRoads.sort((a, b) => a.roadDist - b.roadDist);
      bestOdp = withRoads[0];
      finalRoadDist = bestOdp.roadDist;
    }
  } catch (eOsrm) {
    Logger.log("OSRM Routing Error (Fallback ke straight line): " + eOsrm.message);
  }

  let distToUse = finalRoadDist !== null ? finalRoadDist : Math.round(bestOdp.straightDist);
  let kapasitas = parseInt(bestOdp.kapasitas) || 8;
  let terpakai = parseInt(bestOdp.port_terpakai || bestOdp.portTerpakai) || 0;
  let sisaPort = Math.max(0, kapasitas - terpakai);

  let namaOdp = bestOdp.kode_odp || bestOdp.label || "ODP Terdekat";

  let badgeStatus = "";
  if (distToUse <= 150) {
    badgeStatus = sisaPort > 0 
      ? `🟢 <b>Status :</b> Tercover (± ${distToUse} meter)`
      : `🟡 <b>Status :</b> Tercover (Port Penuh) (± ${distToUse} meter)`;
  } else if (distToUse <= 300) {
    badgeStatus = `🟡 <b>Status :</b> Tercover Jarak Jauh (± ${distToUse} meter)`;
  } else {
    badgeStatus = `🔴 <b>Status :</b> Tidak Tercover (± ${distToUse} meter)`;
  }

  let odpInfo = `🎯 <b>ODP Terdekat  :</b> ${namaOdp}  |  ${sisaPort} / ${kapasitas} port tersedia`;

  let msg = `📡 <b>COVERAGE ALPRO:</b>\n`;
  msg += `${badgeStatus}\n`;
  msg += `${odpInfo}`;

  return {
    isCovered: distToUse <= 300,
    labelOdp: namaOdp,
    distToUse: distToUse,
    sisaPort: sisaPort,
    formattedText: msg,
...
// ==========================================
// FUNGSI NOTIFIKASI REGISTRASI BARU (GROUP BY STASIUN)
// ==========================================
function cekDanKirimNotifRegistrasi() {
  try {
    if (!BOT_TOKEN || BOT_TOKEN === "MASUKKAN_TOKEN_BOT_DISINI") return;
    
    const ssMaster = SpreadsheetApp.openById(SPREADSHEET_ID);
    const dashSheet = ssMaster.getSheetByName("Dashboard");
    if (!dashSheet) return;

    // 1. Ambil data pelanggan baru dari Dashboard O6:R
    const lastRow = dashSheet.getLastRow();
    if (lastRow < 6) return; 
    
    const dataBaru = dashSheet.getRange(6, 15, lastRow - 5, 4).getValues();
    
    // 2. Siapkan Master Database untuk lookup detail pelanggan
    const dbSheet = ssMaster.getSheetByName(typeof DB_SHEET_NAME !== 'undefined' ? DB_SHEET_NAME : "Master_Database");
    const dbMap = {};
    if (dbSheet) {
      const dbValues = dbSheet.getDataRange().getValues();
      if (dbValues.length > 0) {
        const dbHeaders = dbValues.shift();
        dbValues.forEach(r => {
          let obj = {};
          ...
          let id = String(obj['id pelanggan'] || obj['id'] || '').trim();
          if (id) dbMap[id] = obj;
        });
      }
    }
    ...
    // 3. Siapkan Log Sheet
    let logSheet = ssMaster.getSheetByName("Log_Notif_Registrasi");
    if (!logSheet) {
      logSheet = ssMaster.insertSheet("Log_Notif_Registrasi");
      logSheet.appendRow(["ID Pelanggan", "Timestamp Notif"]);
    }
    const logData = logSheet.getRange("A:A").getValues().flat().filter(String);
    const logSet = new Set(logData); 
    
    // 4. Siapkan Mapping Leader
    let mapSheet = ssMaster.getSheetByName("Mapping_Leader");
    if (!mapSheet) {
      mapSheet = ssMaster.insertSheet("Mapping_Leader");
      mapSheet.appendRow(["Stasiun", "Chat ID Telegram", "Username Telegram", "Nama Leader"]);
      Logger.log("Sheet Mapping_Leader baru dibuat.");
      return;
    }
    const mapValues = mapSheet.getDataRange().getValues();
    const leaderMap = {};
    for (let i = 1; i < mapValues.length; i++) {
      let st = String(mapValues[i][0]).toLowerCase().trim();
      let chatId = String(mapValues[i][1]).trim();
      let username = String(mapValues[i][2]).trim();
      let nama = String(mapValues[i][3]).trim();
      if (st && chatId) {
        if (!leaderMap[st]) leaderMap[st] = [];
        leaderMap[st].push({ chatId: chatId, username: username, nama: nama || "Team Leader" });
      }
    }
    
    // 5. Kelompokkan Pelanggan Baru (yang belum dikirim notifnya)
    const groupPerStasiun = {};
    let logsToWrite = [];
    const timestamp = new Date();
    const odpCacheMap = {};
    
    for (let i = 0; i < dataBaru.length; i++) {
      let idPelanggan = String(dataBaru[i][0]).trim();
      let nama = String(dataBaru[i][1]).trim();
      let stasiun = String(dataBaru[i][2]).trim();
      let rawTgl = dataBaru[i][3];
      
      let tgl = "";
      if (rawTgl instanceof Date) {
        tgl = Utilities.formatDate(rawTgl, "GMT+7", "dd MMM yyyy HH:mm") + " WIB";
      } else {
        tgl = String(rawTgl || '').trim();
      }
      
      if (!idPelanggan || idPelanggan === "-") continue;
      
      // Jika belum ada di log, masukkan ke antrean notif
      if (!logSet.has(idPelanggan)) {
        let stKey = stasiun.toLowerCase();
        
        // Ambil detail dari dbMap
        let detail = dbMap[idPelanggan] || {};
        
        // Cek status aktivasi (bypass notifikasi jika sudah aktif)
        let statusAktivasi = String(detail['aktivasi'] || detail['status aktivasi'] || detail['status'] || detail['status akt'] || '').toLowerCase().trim();
        if (statusAktivasi === 'sudah' || statusAktivasi === 'aktif' || statusAktivasi === 'selesai' || statusAktivasi === 'done') {
          logsToWrite.push([idPelanggan, timestamp]);
          continue; 
        }
        
        if (!groupPerStasiun[stKey]) {
          groupPerStasiun[stKey] = [];
        }
        
        let alamat = detail['alamat'] || '-';
        let hp = detail['no hp'] || detail['no. hp'] || detail['nohp'] || detail['no telepon'] || detail['kontak'] || detail['whatsapp'] || detail['nomor hp'] || detail['hp'] || '-';
        let koordinat = detail['titik koordinat'] || detail['koordinat'] || detail['sharelok'] || detail['maps'] || detail['link maps'] || '-';
        let latitude = detail['latitude'] || detail['lat'] || '';
        let longitude = detail['longitude'] || detail['lon'] || detail['lng'] || '';
        
        // Analisa Coverage Jaringan & ODP Terdekat (Roads)
        let coverage = analisaCoverageOdp(stasiun, latitude, longitude, koordinat, ssMaster, odpCacheMap);

        groupPerStasiun[stKey].push({ 
          id: idPelanggan, 
          nama: nama, 
          tgl: tgl,
          stasiunOriginal: stasiun,
          alamat: alamat,
          hp: hp,
          koordinat: koordinat,
          latitude: latitude,
    ...
    // 6. Kirim Notif Telegram & Catat Log
    for (const stKey in groupPerStasiun) {
      const pendaftar = groupPerStasiun[stKey];
      const leaders = leaderMap[stKey];
      
      if (leaders && leaders.length > 0) {
        leaders.forEach(leader => {
          let sapaan = leader.username ? `@${leader.username.replace('@', '')}` : `<b>${leader.nama}</b>`;
          let msg = `✨ <b>REGISTRASI PELANGGAN BARU</b> ✨\n\n`;
          msg += `Halo ${sapaan}, ada pelanggan baru yang terdaftar di stasiunmu menunggu di-follow up :\n\n`;
          
          pendaftar.forEach((p, idx) => {
            if (idx > 0) msg += `----------------------------\n\n`;
            
            // Format WA link
            let waLink = p.hp.replace(/\D/g, '');
            if (waLink.startsWith('0')) waLink = '62' + waLink.substring(1);
            let waText = p.hp !== '-' && waLink.length > 5 ? `<a href="https://wa.me/${waLink}">${p.hp}</a>` : '-';
            
            // Titik Koordinat (Mode Code / Monospace)
            let mapText = "-";
            let coords = p.coverage && p.coverage.coords ? p.coverage.coords : null;
            if (coords) {
              mapText = `<code>${coords.lat}, ${coords.lng}</code>`;
            } else if (p.koordinat && p.koordinat !== '-') {
              mapText = `<code>${p.koordinat}</code>`;
            } else if (p.latitude && p.longitude) {
              mapText = `<code>${p.latitude}, ${p.longitude}</code>`;
            }

            msg += `👤 <b>${pendaftar.length > 1 ? (idx + 1) + '. ' : ''}${p.nama.toUpperCase()}</b>\n`;
            msg += `💳 <b>ID Pelanggan :</b> <code>${p.id}</code>\n`;
            msg += `📍 <b>Stasiun      :</b> ${p.stasiunOriginal}\n`;
            msg += `🏠 <b>Alamat       :</b> ${p.alamat}\n`;
            msg += `📞 <b>No. HP       :</b> ${waText}\n`;
            msg += `🗺️ <b>Koordinat    :</b> ${mapText}\n`;
            if (p.tgl) msg += `🕒 <b>Waktu Reg.   :</b> <i>${p.tgl}</i>\n`;
            
            // Masukkan Analisa Coverage ODP
            if (p.coverage && p.coverage.formattedText) {
              msg += `\n${p.coverage.formattedText}\n`;
            }
            msg += `\n`;
          });
          
          msg += `⚡ <i>Segera jadwalkan tim untuk survey & aktivasi ya!</i>`;
          
          try {
            UrlFetchApp.fetch("https://api.telegram.org/bot" + BOT_TOKEN + "/sendMessage", {
              method: "post",
              contentType: "application/json",
              payload: JSON.stringify({ chat_id: leader.chatId, text: msg, parse_mode: "HTML", disable_web_page_preview: true }),
              muteHttpExceptions: true
            });
          } catch (e) {
            Logger.log("Gagal kirim telegram ke " + stKey + ": " + e.message);
          }
        });
        
        // Jika berhasil dikirim, catat untuk Log
        pendaftar.forEach(p => {
          logsToWrite.push([p.id, timestamp]);
        });
      }
    }
    
    // 7. Simpan Log ke Sheet agar tidak berulang
    if (logsToWrite.length > 0) {
      logSheet.getRange(logSheet.getLastRow() + 1, 1, logsToWrite.length, 2).setValues(logsToWrite);
    }
    
  } catch (err) {
    Logger.log("Error di cekDanKirimNotifRegistrasi: " + err.message);
  }
}

// ==========================================
// FUNGSI UJI COBA (TRIAL) KE TELEGRAM PRIBADI
// ==========================================
function trialKirimPesan() {
  // 1. GANTI ANGKA DI BAWAH INI DENGAN CHAT ID ANDA SENDIRI
  const CHAT_ID_SAYA = "163140606"; 
  
  try {
    if (!BOT_TOKEN || BOT_TOKEN === "MASUKKAN_TOKEN_BOT_DISINI") {
      Logger.log("BOT_TOKEN belum disetting!");
      return;
    }
    
    let msg = `📢 <b>UJI COBA SISTEM NOTIFIKASI</b> 📢\n\n`;
    msg += `Halo Developer,\nIni adalah pesan percobaan dari sistem <b>OpsTracker</b>.\n\n`;
    msg += `✅ Jika Anda menerima pesan ini, artinya Bot Telegram sudah terhubung dengan baik ke script!\n\n`;
    msg += `<i>~ Salam dari Antigravity ~</i>`;
    
    UrlFetchApp.fetch("https://api.telegram.org/bot" + BOT_TOKEN + "/sendMessage", {
      method: "post",
      contentType: "application/json",
      payload: JSON.stringify({ chat_id: CHAT_ID_SAYA, text: msg, parse_mode: "HTML" }),
      muteHttpExceptions: true
    });
    
    Logger.log("Pesan uji coba berhasil dikirim ke " + CHAT_ID_SAYA);
    
  } catch (e) {
    Logger.log("Gagal mengirim pesan uji coba: " + e.message);
  }
}
