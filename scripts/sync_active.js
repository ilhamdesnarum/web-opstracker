// =========================================================================
// SCRIPT SINKRONISASI PELANGGAN AKTIF MASSAL -> SUPABASE (NODE.JS)
// =========================================================================
// Khusus menarik seluruh data Pelanggan Aktif (Active)
// =========================================================================
...
async function main() {
  const t0 = Date.now();
  console.log("=================================================================");
  console.log("📦 MEMULAI SINKRONISASI PELANGGAN AKTIF (MASSAL) -> SUPABASE");
  console.log("=================================================================");

  const targetStations = process.env.SYNC_STATIONS && process.env.SYNC_STATIONS !== 'ALL'
  ...
  for (const stationName of targetStations) {
    ...
    while (hasMore) {
      ...
      try {
        ...
        console.log(`   ↳ Hal ${page}: ${customers.length} data (Total stasiun: ${statusCount})`);
        ...
        console.log(`   ↳ JSON Parse Error pada halaman ${page}`);
        hasMore = false;
      }
    }
  ...
  console.log("\n=================================================================");
  console.log(`🎉 SINKRONISASI PELANGGAN AKTIF SELESAI DALAM ${durationMin} MENIT!`);
  console.log(`📊 Total Pelanggan Aktif Ditarik  : ${grandTotalFetched}`);
